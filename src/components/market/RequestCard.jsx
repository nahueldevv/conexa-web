...
function RequestCard({ item }) {
  ...
  return (
    <div
      className="rounded-xl border border-cyan-400 bg-white p-5 shadow-sm 
                 transition-all duration-300 hover:shadow-md dark:border-cyan-500 
                 dark:bg-neutral-900"
    >
      {/* Header */}
      ...
      {expanded && (
        <div className="mt-3 rounded-lg bg-neutral-100 dark:bg-neutral-800 p-3 text-sm space-y-1 transition-all duration-300">
          <p><span className="font-medium">Temperatura requerida:</span> {item?.requiredTemp || "N/A"}</p>
          <p><span className="font-medium">Presupuesto:</span> {item?.budget || "N/A"}</p>
          <p><span className="font-medium">Vehículos necesarios:</span> {item?.vehicles || "N/A"}</p>
          <p><span className="font-medium">Notas:</span> {item?.notes || "N/A"}</p>
        </div>
      )}
